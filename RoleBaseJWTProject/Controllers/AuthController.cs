...
namespace RoleBaseJWTProject.Controllers
{
    ...
    private readonly IConfiguration _configuration;
        ...
        public async Task<IActionResult> SignUp(SignUpDto dto)
        {
            ...
            if (result.Succeeded)
            {
                if (!await _roleManager.RoleExistsAsync("Admin"))//like is there any role as "Admin" in whole system?
                {//this block will only work for the first initial time, and never again
                    await _roleManager.CreateAsync(new IdentityRole("Admin"));
                }

                await _userManager.AddToRoleAsync(user, "Admin");//and each time wi will be adding new user
                                                                 //to existing Admin role
